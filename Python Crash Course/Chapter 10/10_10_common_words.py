...
# computer.
# You can use the count() method to find out how many times a
# word or phrase appears in a string. For example, the
# following code counts the number of times 'row' appears in a
# string:
...
bookfolder = Path(__file__).parent / 'Textfiles/Books'
files = [file for file in bookfolder.glob("*.txt")]
searches = [ 'the', 'the ', ' the', ' the ' ]

for search in searches:
    for file in files:
        content = file.read_text(encoding='utf-8')
        count = content.lower().count(search)
        print(f"'{search}' occurs {count}x in {file.name}")
    print()